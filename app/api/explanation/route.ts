...
export async function POST(req: NextRequest) {
  try {
    ...
    // 构建解释请求
    const explanationPrompt = `请对以下日文句子进行详细分析，以帮助学习者深入理解。请严格按照以下三个部分组织内容，并使用Markdown格式：

### 1. 句子翻译
将下面的日文句子翻译成通顺的简体中文。

### 2. 重点语法分析
分析句子中的关键语法点。例如，解释助词的用法、动词的时态和形态变化、句型结构等。请使用列表或清晰的段落进行说明。

### 3. 重点单词分析
列出句子中的主要单词，并为每个单词提供以下信息：
*   **读音**: 单词的平假名读音。
*   **词性**: 单词的词性。
*   **基本形式**: 如果是动词或形容词，提供其字典形式。
*   **中文意思**: 提供核心的中文意思。

原文：
${text}

请确保输出内容清晰、有条理，并且严格遵循上述三部分结构。`;
    const payload = {
      model: model,
